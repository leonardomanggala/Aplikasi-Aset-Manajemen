    ...
    <div className="min-h-screen bg-slate-50/80 flex flex-col md:flex-row antialiased font-sans">
      ...
      <aside className={`simas-sidebar relative fixed inset-y-0 left-0 z-50 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 w-72 md:w-auto ${isSidebarCollapsed ? 'md:w-16' : 'md:w-56'} bg-slate-950 flex flex-col border-r border-slate-800/50 text-slate-300 shrink-0 select-none md:sticky md:top-0 h-screen transition-all duration-300 ease-in-out shadow-2xl shadow-slate-950/20`}>
        ...
        <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto">
          {!isSidebarCollapsed && (
            <div className="text-xs text-slate-500 font-semibold tracking-wide mb-3 px-2">Menu Utama</div>
          )}
          
          <button
            onClick={() => handleTabChange('dashboard')}
            title="Dashboard Rekapitulasi"
            className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center py-3' : 'gap-3 py-2.5 px-3'} rounded-lg text-[16px] font-semibold tracking-normal transition ${
              activeTab === 'dashboard'
          ...
          <div className="space-y-1">
            <button
              ...
              className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center py-3' : 'justify-between py-2.5 px-3'} rounded-lg text-[16px] font-semibold tracking-normal transition relative ${
              ...
              {isAssetMenuExpanded && !isSidebarCollapsed && (
                ...
                >
                  <button
                    ...
                    className={`w-full flex items-center justify-between py-2 px-3 rounded-lg text-[16px] font-semibold tracking-normal transition ${
          ...
          <button
            ...
            className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center py-3 text-primary-500 hover:bg-slate-800/50' : 'gap-3 py-2.5 px-3'} rounded-lg text-[16px] font-semibold tracking-normal transition ${
              activeTab === 'qr'
                ? 'bg-primary-500/10 text-primary-400 font-bold ring-1 ring-primary-500/30 shadow-lg shadow-primary-500/20'
                : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'
            }`}
