...
interface AssetListTabProps {
  ...
  assetCategoryFilter?: 'bergerak' | 'tidak_bergerak' | 'all';
}
...
export default function AssetListTab({
  ...
  const [formData, setFormData] = useState({
    ...
    qty: 1,
    satuan: 'pcs',
    kategoriAset: 'bergerak' as 'bergerak' | 'tidak_bergerak' | undefined,
    ...
    certificateNumber: undefined as string | undefined,
    propertyRights: undefined as string | undefined,
    ...
    buildingPermit: undefined as string | undefined
  });
  ...
  const autoCalculatedUrut = useMemo(() => {
    if (isEditing) return formData.noUrutSejenis;
    const baseKey = `${formData.jenisAset}-${formData.teritori}-${formData.peruntukan}-${formData.letakRuang}-${formData.kodeNamaBarang}`;
    let maxUrut = 0;
    ...
    return String(maxUrut + 1);
  }, [isEditing, formData.jenisAset, formData.teritori, formData.peruntukan, formData.letakRuang, formData.kodeNamaBarang, formData.noUrutSejenis, assets]);

  // Preview automatic calculation on form changes
  const livePreview = useMemo(() => {
    const parsedYear = new Date(formData.tanggalPerolehan).getFullYear();
    const serial = generateNoSeriFinal({
      ...
      tahun: !isNaN(parsedYear) ? parsedYear : 2024,
      tanggalPerolehan: formData.tanggalPerolehan,
      noUrutSejenis: autoCalculatedUrut
    });
    ...
    const valResult = calculateStraightLineDepreciation(
      ...
      isAssetDepreciable(formData as Partial<Asset>)
    );
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter(asset => {
      let matchSubMenu = true;
      if (assetCategoryFilter === 'tidak_bergerak') {
        matchSubMenu = asset.kategoriAset === 'tidak_bergerak';
      } else if (assetCategoryFilter === 'bergerak') {
        matchSubMenu = asset.kategoriAset === 'bergerak' || asset.kategoriAset === undefined;
      }

      const term = searchTerm.toLowerCase().trim();
      const searchTerms = term.split(/\s+/).filter(Boolean);
      const searchTarget = `${asset.uraian} ${asset.noSeriFinal} ${jMap[asset.jenisAset] || ''} ${lMap[asset.letakRuang] || ''}`.toLowerCase();
      
      const matchSearch = searchTerms.length === 0 || searchTerms.every(t => searchTarget.includes(t));
      ...
      return matchSubMenu && matchSearch && matchCategory && matchLocation && matchCondition && matchBidang;
    });
  }, [assets, searchTerm, selectedCategory, selectedLocation, selectedCondition, selectedBidang, jMap, lMap, assetCategoryFilter]);
  ...
  const handleOpenEditForm = (asset: Asset) => {
    ...
    setFormData({
      ...
      kategoriAset: asset.kategoriAset || 'bergerak',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      ...
      const updated: Asset = {
        ...
        isDepreciable: isAssetDepreciable(formData as Partial<Asset>),
      ...
      Object.keys(created).forEach(key => {
        if ((created as any)[key] === undefined) {
          delete (created as any)[key];
        }
      });
      onAddAsset(created);
    }
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    const csvCategoryName = assetCategoryFilter === 'bergerak' ? '-bergerak' : assetCategoryFilter === 'tidak_bergerak' ? '-tidak-bergerak' : '';
    link.download = `register-aset-pringwulung${csvCategoryName}_${dateStr}_${timeStr}.csv`;
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    const pdfTitle = `Laporan Daftar Aset ${assetCategoryFilter === 'bergerak' ? '- Bergerak' : assetCategoryFilter === 'tidak_bergerak' ? '- Tidak Bergerak' : ''}`;
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex justify-end">
          <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col justify-between overflow-hidden animate-slide-in-right">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              {/* Kategori Aset (Bergerak / Tidak Bergerak) */}
              <div className="space-y-1">
                <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Kategori Aset</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { value: 'bergerak', label: 'Barang Bergerak' },
                    { value: 'tidak_bergerak', label: 'Barang Tidak Bergerak' }
                  ].map(cat => (
                    <button
                      key={cat.value}
                      type="button"
                      onClick={() => setFormData({...formData, kategoriAset: cat.value as 'bergerak' | 'tidak_bergerak'})}
                      className={`text-xs font-semibold p-2.5 rounded-lg border text-center transition cursor-pointer ${
                        formData.kategoriAset === cat.value 
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg focus:outline-none"
                  />
                </div>
                <div className="space-y-1">
              ...
              {formData.jenisAset === '100' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-100">
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Luas Tanah (m²)</label>
                    <input 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Nomor Sertifikat</label>
                    <input 
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Jenis Hak (cth: SHM, HGB)</label>
                    <input 
                      type="text"
                      value={formData.propertyRights || ''}
                      onChange={(e) => setFormData({...formData, propertyRights: e.target.value})}
                      className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg focus:outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Lokasi / Alamat</label>
                    <input 
                      type="text"
              ...
              {formData.jenisAset === '101' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-100">
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Luas Bangunan (m²)</label>
                    <input 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Jumlah Lantai</label>
                    <input 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">Tahun Dibangun</label>
                    <input 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-bold block uppercase tracking-wide">No. PBG / IMB</label>
                    <input 
                      type="text"
                      value={formData.buildingPermit || ''}
                      onChange={(e) => setFormData({...formData, buildingPermit: e.target.value})}
                      className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg focus:outline-none"
                    />
                  </div>
                </div>
              ...
              <div className="bg-slate-950 text-slate-100 p-4 rounded-xl border border-slate-800 space-y-1.5 shadow-inner">
                ...
                <div className="flex justify-between items-center text-[11px] pt-1">
                  ...
                  <span className="font-bold font-mono text-amber-500">{livePreview.umurBerjalanTahun} Tahun</span>
                </div>
