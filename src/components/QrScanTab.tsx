...
export default function QrScanTab({
  ...
  const startCamera = async () => {
    const BarcodeDetector = (window as Window & { BarcodeDetector?: NativeBarcodeDetectorConstructor }).BarcodeDetector;

    if (!navigator.mediaDevices?.getUserMedia) {
      setScanMessage('Kamera tidak didukung browser ini. Gunakan input nomor seri manual.');
      return;
    }

    if (!BarcodeDetector) {
      setScanMessage('Pemindaian QR otomatis belum didukung browser ini. Kamera dapat digunakan untuk melihat label, lalu masukkan nomor seri secara manual.');
    }
    ...
    try {
      ...
      setScanMessage(BarcodeDetector ? 'Kamera aktif. Arahkan label QR ke kotak pemindai.' : 'Kamera aktif. Browser ini membutuhkan input nomor seri manual.');
      ...
      requestAnimationFrame(async () => {
        ...
        const detectFrame = async () => {
          ...
          try {
            ...
            if (codes[0]?.rawValue) {
              stopCamera();
              executeScan(codes[0].rawValue);
              return;
            }
          } catch {
            // A frame without a readable QR is normal; keep scanning.
          }
      ...
      setScanMessage('Izin kamera ditolak atau kamera tidak tersedia. Gunakan input nomor seri manual.');
      stopCamera();
    }
  };
  ...
  const deprValues = useMemo(() => {
    ...
    return calculateStraightLineDepreciation(
      ...
      isAssetDepreciable(scannedAsset)
    );
  }, [scannedAsset]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 space-y-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
          ...
          {/* Native camera preview with a visual scanning guide */}
          <div className="relative bg-slate-950 aspect-[4/3] rounded-xl overflow-hidden border border-slate-900 flex flex-col justify-between items-center p-3 text-center">
            <video
              ...
              muted
              playsInline
              className={`absolute inset-0 h-full w-full object-cover transition-opacity ${isCameraLive ? 'opacity-100' : 'opacity-0'}`}
            />
            <div className={`absolute inset-0 bg-slate-950 transition-opacity ${isCameraLive ? 'opacity-35' : 'opacity-100'}`}></div>
            ...
            <div className="self-end z-10 bg-black/60 backdrop-blur text-white px-2.5 py-0.5 rounded text-[9px] font-mono select-none flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${isCameraLive ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`}></div>
              <span>{isCameraLive ? 'KAMERA AKTIF' : 'KAMERA SIAGA'}</span>
            </div>
            ...
            <div className="self-start z-10 text-[8px] text-slate-400 font-mono tracking-tight text-left">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="rounded-lg bg-primary-600 px-3 py-2.5 text-xs font-bold text-white transition hover:bg-primary-700"
            >
              {isCameraLive ? 'Matikan Kamera' : 'Aktifkan Kamera'}
            </button>
            <span className="flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 px-2 text-center text-[10px] text-slate-500">
              QR native jika browser mendukung
            </span>
          </div>

          {/* Fallbacks for browsers and desktop scanners */}
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <div className="space-y-1">
              ...
              <select 
                ...
                className="w-full text-xs p-2.5 bg-slate-50 dark:bg-slate-900/40 border dark:border-slate-800 border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary-500 font-mono"
              >
                <option value="">-- Pilih aset untuk uji cepat --</option>
                {assets.map(asset => (
      ...
      <div className="lg:col-span-8">
        {!scannedAsset ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-full">
            ...
            <div className="p-5 border-b border-slate-100 bg-slate-50/60 dark:bg-slate-900/40 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-slate-100 text-xs bg-slate-50/20 dark:bg-slate-900/40 dark:border-slate-800">
            ...
            <div className="px-5 py-3 bg-slate-50/50 dark:bg-slate-900/40 dark:border-slate-800 border-b border-slate-200 text-xs text-slate-600 grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="flex-1 p-5 overflow-y-auto">
              ...
              {activeTab === 'maintenance' && (
                <div className="space-y-4">
                  ...
                  {showLogForm && (
                    <form onSubmit={handleAddLog} className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-primary-100 space-y-3 animate-fade-in text-xs">
                  ...
                  {!scannedAsset.maintenanceLogs || scannedAsset.maintenanceLogs.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {scannedAsset.maintenanceLogs.map(log => (
                        <div key={log.id} className="bg-slate-50 dark:bg-slate-900/40 border dark:border-slate-800 border-slate-100 p-3.5 rounded-lg text-xs space-y-1.5">
              ...
              {activeTab === 'mutations' && (
                <div className="space-y-4">
                  ...
                  {showMutationForm && (
                    <form onSubmit={handleAddMut} className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-primary-100 space-y-3 animate-fade-in text-xs">
                  ...
                  {!scannedAsset.mutations || scannedAsset.mutations.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {scannedAsset.mutations.map(mut => (
                        <div key={mut.id} className="bg-slate-50 dark:bg-slate-900/40 border dark:border-slate-800 border-slate-100 p-3 rounded-lg text-xs flex items-start gap-3">
              ...
              {activeTab === 'documents' && (
                <div className="space-y-4">
                  ...
                  {showDocForm && (
                    <form onSubmit={handleAddDoc} className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-primary-100 space-y-3 animate-fade-in text-xs">
                  ...
                  {!scannedAsset.documents || scannedAsset.documents.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {scannedAsset.documents.map(doc => (
                        <div key={doc.id} className="p-3 bg-slate-50 dark:bg-slate-900/40 border dark:border-slate-800 border-slate-100 rounded-lg flex items-center justify-between text-xs hover:border-primary-200 transition">
