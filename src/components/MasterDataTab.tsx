...
export default function MasterDataTab({
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(importedData)) {
          ...
          syncAllAssetsToFirebase(importedData).catch(console.error);
          setSuccessMsg('Data JSON berhasil diimpor.');
          setTimeout(() => setSuccessMsg(''), 3000);
        } else {
          setFormError('Format JSON tidak valid, diharapkan array of assets.');
        }
      } catch (err) {
        setFormError('Gagal mengurai file JSON.');
      }
  ...
  const handleSubmitForm = (e: React.FormEvent) => {
    ...
    const getMapNameForTab = (tab: string) => {
      switch (tab) {
        case 'jenis': return 'jenisAsetMap';
        case 'ruang': return 'letakRuangMap';
        case 'teritori': return 'teritoriMap';
    ...
    const mapName = getMapNameForTab(subTab);
    const deletedCodes = (editingCode && editingCode !== codeClean) ? [editingCode] : [];
    updateMasterDataAtomic(mapName, { [codeClean]: nameClean }, deletedCodes).catch(console.error);
        ...
        <div className="flex items-center gap-2 flex-wrap md:flex-nowrap">
          ...
          <button
            onClick={() => {
              ...
              const exportFileDefaultName = `backup-aset-${new Date().toISOString().slice(0,10)}.json`;
              const linkElement = document.createElement('a');
              linkElement.setAttribute('href', dataUri);
              linkElement.setAttribute('download', exportFileDefaultName);
              linkElement.click();
            }}
            className="bg-primary-600 text-white hover:bg-primary-700 text-xs py-1.5 px-3 rounded-lg flex items-center gap-1.5 font-semibold transition cursor-pointer text-nowrap"
          >
            ...
            <div className="flex gap-2.5 pt-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  updateMasterDataAtomic(getMapNameForTab(subTab), {}, [deleteRef.code]).catch(console.error);
            ...
            <div className="flex gap-2.5 pt-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  syncMasterDataToFirebase({ [getMapNameForTab(subTab)]: originalMapBaseline }).catch(console.error);
